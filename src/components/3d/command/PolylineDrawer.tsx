...
import { polylinePointsAtom, createNewPolylineAtom } from "@/stores/points"
...
import { pointNodeIdAtom } from "@/stores/modular"
...
const PolylineDrawer = () => {
  ...
  const onPointerDown = useCallback(
    (e: ThreeEvent<PointerEvent>) => {
      if (e.button === 0) {
        const currentPoints = [...points, currentCursorPoint]
  ...
  const onPointerMove = useCallback(
    (e: ThreeEvent<PointerEvent>) => {
      ...
      if (points.length > 0) {
        ...
        const {
          snapDirection,
          guidelinePoints: newGuidelinePoints,
          snappedPoint,
        } = detectSnapDirection(previousPoint, currentPt)
  ...
  //polylineAtomに変更があったら更新
  useEffect(() => {
    if (!polylines) return
    updateNodeProperty({
      id: pointNodeId!,
      value: `{"points":${JSON.stringify(polylines)}}`,
    })
  }, [polylines])
